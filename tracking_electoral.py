...
# ── El resto del Bloque 7 es idéntico al original
...
if tipo_track == "d":
    margen_de_error_img = (
        df.groupby('Ventana_D').apply(lambda g: margen_error_imagen(g, 'peso_d')).reset_index()
    )
    print("=== Intervalos de confianza — Imagen (diario) (95%) ===")
    print(margen_de_error_img)
    margen_de_error_vot = (
        df.groupby('Ventana_D').apply(lambda g: margen_error_voto(g, 'peso_d')).reset_index()
    )
    print("=== Intervalos de confianza — Voto (diario) (95%) ===")
    print(margen_de_error_vot)
elif tipo_track == "s":
    margen_de_error_img = (
        df.groupby('Ventana_S').apply(lambda g: margen_error_imagen(g, 'peso_s')).reset_index()
    )
    print("=== Intervalos de confianza — Imagen (semanal) (95%) ===")
    print(margen_de_error_img)
    margen_de_error_vot = (
        df.groupby('Ventana_S').apply(lambda g: margen_error_voto(g, 'peso_s')).reset_index()
    )
    print("=== Intervalos de confianza — Voto (semanal) (95%) ===")
    print(margen_de_error_vot)
else:
    margen_de_error_img = (
        df.groupby('Ventana_M').apply(lambda g: margen_error_imagen(g, 'peso_m')).reset_index()
    )
    print("=== Intervalos de confianza — Imagen (mensual) (95%) ===")
    print(margen_de_error_img)
    margen_de_error_vot = (
        df.groupby('Ventana_M').apply(lambda g: margen_error_voto(g, 'peso_m')).reset_index()
    )
    print("\n=== Intervalos de confianza — Voto (mensual) (95%) ===")
    print(margen_de_error_vot)


# %%
# Decimotercer paso: Test estadístico de cambio en la imagen
...
if normalidad and homocedasticidad:
    ...
    raise ValueError("No se puede realizar un test válido")
